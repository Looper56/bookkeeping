...
func (qrCode *QRCode) CreateWXAQRCode(coderParams QRCoder) (response []byte, err error) {
	return qrCode.fetchCode(api.CreateWXAQRCodeUri(), coderParams)
}
...
func (qrCode *QRCode) GetWXACode(coderParams QRCoder) (response []byte, err error) {
	return qrCode.fetchCode(api.GetWXACodeUri(), coderParams)
}

// GetWXACodeUnlimited 获取小程序码，适用于需要的码数量极多的业务场景
// 文档地址： https://developers.weixin.qq.com/miniprogram/dev/api/getWXACodeUnlimit.html
func (qrCode *QRCode) GetWXACodeUnlimited(coderParams QRCoder) (response []byte, err error) {
	return qrCode.fetchCode(api.GetWXACodeUnlimitedUri(), coderParams)
}
