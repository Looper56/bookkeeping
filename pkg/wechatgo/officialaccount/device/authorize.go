...
type ReqDevice struct {
	...
	//	1：表示设备仅支持andiod classic bluetooth 1|2：表示设备支持android 和ios 两种classic bluetooth，但是客户端优先
	//	选择android classic bluetooth 协议，如果android classic bluetooth协议连接失败，再选择ios classic bluetooth协议进行连接
	...
	// 连接策略，32位整型，按bit位置位，目前仅第1bit和第3bit位有效（bit置0为无效，1为有效；第2bit已被废弃），
	// 且bit位可以按或置位（如1|4=5），各bit置位含义说明如下：
	...
	// auth version，设备和微信进行auth时，会根据该版本号来确认auth buf和auth key的格式（各version对应的auth buf及key的
	// 具体格式可以参看“客户端蓝牙外设协议”），该字段目前支持取值：
	// 0：不加密的version
	// 1：version 1
...
func (d *Device) DeviceAuthorize(devices []ReqDevice, opType int, product string) (res []ResBaseInfo, err error) {
	...
	uri := api.UriAuthorizeUri(accessToken)
