...
func (menu *Menu) SetMenu(buttons []*Button) error {
	...
	uri := api.MenuCreateUri(accessToken)
	reqMenu := &reqMenu{
		Button: buttons,
	}
...
func (menu *Menu) GetMenu() (resMenu ResMenu, err error) {
	...
	uri := api.MenuGetUri(accessToken)
	var response []byte
...
func (menu *Menu) DeleteMenu() error {
	...
	uri := api.MenuDeleteUri(accessToken)
	response, err := util.HTTPGet(uri)
	if err != nil {
		return err
	}
...
func (menu *Menu) AddConditional(buttons []*Button, matchRule *MatchRule) error {
	...
	uri := api.MenuAddConditionalUri(accessToken)
	reqMenu := &reqMenu{
		Button:    buttons,
		MatchRule: matchRule,
	}
...
func (menu *Menu) DeleteConditional(menuID int64) error {
	...
	uri := api.MenuDeleteConditionalUri(accessToken)
...
func (menu *Menu) MenuTryMatch(userID string) (buttons []Button, err error) {
	...
	uri := api.MenuTryMatchUri(accessToken)
...
func (menu *Menu) GetCurrentSelfMenuInfo() (resSelfMenuInfo ResSelfMenuInfo, err error) {
	...
	uri := api.MenuSelfMenuInfoUri(accessToken)
	var response []byte
	response, err = util.HTTPGet(uri)
	if err != nil {
		return
	...
	if resSelfMenuInfo.ErrCode != 0 {
		err = fmt.Errorf("GetCurrentSelfMenuInfo Error , errcode=%d , errmsg=%s", resSelfMenuInfo.ErrCode,
			resSelfMenuInfo.ErrMsg)
		return
	}
	return
}
