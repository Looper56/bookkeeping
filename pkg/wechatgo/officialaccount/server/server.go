...
func (srv *Server) handleRequest() (reply *message.Reply, err error) {
	...
	if mixMessage.InfoType == message.InfoTypeVerifyTicket {
		// set verify ticket
		err = srv.Context.SetVerifyTicket(mixMessage.ComponentVerifyTicket)
		// 直接返回success
		_, err = srv.Writer.Write([]byte("success"))
	} else {
