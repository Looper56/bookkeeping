...
func (broadcast *Broadcast) SendText(user *User, content string) (*Result, error) {
	...
	req, url := broadcast.chooseTagOrOpenID(user, req, ak)
...
// chooseTagOrOpenID 发送预览，群发消息状态，发送速度
func (broadcast *Broadcast) chooseTagOrOpenID(user *User, req *sendRequest, ak string) (ret *sendRequest, url string) {
	sendURL := ""
	if user == nil {
		req.Filter = map[string]interface{}{
			"is_to_all": true,
		}
		sendURL = api.SendURLByTagUri(ak)
	} else {
		if user.TagID != 0 {
			req.Filter = map[string]interface{}{
				"is_to_all": false,
		...
		if len(user.OpenID) != 0 {
			...
			sendURL = api.SendURLByOpenIDUri(ak)
		}
	}
	return req, sendURL
}
