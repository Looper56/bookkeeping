...
func (cube *DataCube) GetInterfaceSummary(s string, e string) (resInterfaceSummary ResInterfaceSummary, err error) {
	...
	uri := api.GetInterfaceSummaryUri(accessToken)
	reqDate := &reqDate{
		BeginDate: s,
		EndDate:   e,
	}
...
func (cube *DataCube) GetInterfaceSummaryHour(s string, e string) (resInterfaceSummaryHour ResInterfaceSummaryHour,
	err error) {
	accessToken, err := cube.GetAccessToken()
	if err != nil {
		return
	...
	uri := api.GetInterfaceSummaryHourUri(accessToken)
	reqDate := &reqDate{
		BeginDate: s,
		EndDate:   e,
	}
